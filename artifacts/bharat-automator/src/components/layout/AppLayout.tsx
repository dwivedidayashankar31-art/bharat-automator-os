...
export const AppLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="h-screen w-full bg-background text-foreground flex font-sans overflow-hidden">
      {/* Desktop Sidebar */}
      <div className="hidden md:block w-72 shrink-0 border-r border-border/50 bg-card/40 backdrop-blur-2xl relative z-20">
        <Sidebar />
      </div>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, x: -20 }}
            className="fixed inset-0 top-16 bg-background/95 backdrop-blur-xl z-40 md:hidden border-t border-border/50"
          >
      ...
      <main className="flex-1 relative overflow-y-auto pt-16 md:pt-0">
        ...
        <div 
          className="fixed inset-0 bg-cover bg-center opacity-[0.04] pointer-events-none mix-blend-screen"
          style={{ backgroundImage: `url(${import.meta.env.BASE_URL}images/hero-bg.png)` }}
        />
        
        {/* Top Glow */}
        <div className="fixed top-0 left-0 right-0 h-[500px] bg-gradient-to-b from-primary/5 via-primary/0 to-transparent pointer-events-none -z-10" />
